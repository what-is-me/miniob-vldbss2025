...
RC OrderByLimitVecPhysicalOperator::open(Trx *trx)
{
  ...
  if (order_by_exprs_.size() == 1 && order_by_exprs_.front()->type() == ExprType::AGGREGATION &&
      static_cast<AggregateExpr *>(order_by_exprs_.front().get())->aggregate_type() == AggregateExpr::Type::COUNT &&
      asc_.front() == false) {
    Column column;
    while (OB_SUCC(rc = child.next(chunk_))) {
      order_by_exprs_.front()->get_column(chunk_, column);
      __order_by_count::append_rows_limit(rows2_, chunk_, column, n_);
    }
  } else {
...
RC OrderByLimitVecPhysicalOperator::next(Chunk &chunk)
{
  if (order_by_exprs_.size() == 1 && order_by_exprs_.front()->type() == ExprType::AGGREGATION &&
      static_cast<AggregateExpr *>(order_by_exprs_.front().get())->aggregate_type() == AggregateExpr::Type::COUNT &&
      asc_.front() == false) {
    if (rows2_.empty() || chunk_.capacity() == 0) {
      return RC::RECORD_EOF;
    }
    chunk.reset();
    const int total_rows = rows2_.size();
    ...
    __order_by_count::append_chunk(chunk, rows2_);
    return RC::SUCCESS;
  } else {
    chunk.reset();
